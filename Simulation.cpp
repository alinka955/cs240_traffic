...
void moveCars(std::vector<VehicleBase *> &vehicles, int size, bool greenEW, bool yellowEW, bool redEW, bool greenNS, bool yellowNS, bool redNS)
{
    if (size > 1)
    {
...
int main(int argc, char *argv[])
{
    ...
    while (numClicks < maximum_simulated_time)
    {
        ...
        moveCars(eastbound, halfsize, greenEW, yellowEW, redEW, greenNS, yellowNS, redNS);
        //moveCars(westbound, halfsize);
        //moveCars(northbound, halfsize);
        moveCars(southbound, halfsize,greenEW, yellowEW, redEW, greenNS, yellowNS, redNS);
